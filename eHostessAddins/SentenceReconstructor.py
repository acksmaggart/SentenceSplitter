...
class SentenceReconstructor:
    ...
    def reconstructSentence(self, alteredSentence):
        """
        This method is utilized by passing sentences from the default pyConText sentence splitter in the same
        order that they appear in the document. This method ensures that the sentence is reconstructed to match
        its form in the document so that its document position can be found using a string search. Reconstruction
        is necessary because the default sentence splitter changes whitespace characters found in the original
        document. This method is meant to be used in conjunction with SentenceRepeatManager to ensure that the
        proper document span is returned in the event that the sentence appears multiple times in the document.
